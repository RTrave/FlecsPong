...
//		const auto& player_spr = holder.registery->get<Sprite>(holder.player);
        auto& ball           = holder.world->get<Ball>()[holder.ball];
        const auto& ball_pos = holder.world->get<Position>()[holder.ball];
        const auto& ball_spr = holder.world->get<Sprite>()[holder.ball];

        const auto& ai_pos = holder.world->get<Position>()[holder.ai];
        const auto& ai_spr = holder.world->get<Sprite>()[holder.ai];

        const auto& player_pos = holder.world->get<Position>()[holder.player];
        const auto& player_spr = holder.world->get<Sprite>()[holder.player];
